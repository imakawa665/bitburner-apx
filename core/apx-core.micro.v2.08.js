/** apx-core.micro.v2.08.js
 *  低RAMマネージャ（"allRooted" 有効時、root取得済みサーバを自動ターゲットに含める）
 *  @param {NS} ns
 */
export async function main(ns) {
  ...
  const pickTarget = () => {
    ...
    if (Date.now()-state.lastPick<5000 && state.target) return state.target;
  ...
  while(true){
    ...
    if(!state.paused){
      ...
      else await runFill('apx-h1.js',[tgt],16);
    }
