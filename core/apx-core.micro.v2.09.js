...
export async function main(ns) {
  ...
  const discover = () => {
    ...
    while(q.length){ const cur=q.pop(); for(const n of ns.scan(cur)){ if(!seen.has(n)){ seen.add(n); q.push(n);} } }
  ...
  const candidates = () => {
    ...
    for(const h of discover()){
      if(skip.has(h)) continue; if(!ns.serverExists(h)) continue; if(!ns.hasRootAccess(h)) continue;
  ...
  const pickTarget = () => {
    ...
    let best='',score=-1; for(const h of candidates()){
  ...
  const runFill = async(file,args=[],prefer=1e9)=>{
    const max=ns.getServerMaxRam('home'), used=ns.getServerUsedRam('home');
    let free=Math.max(0,max-used); if (free<1) return 0;
    let th=Math.min(prefer,Math.max(1,Math.floor(free/2))); let started=0;
    while(th>0){ const pid=ns.run(file,th,...args); if(pid!==0){started=th;break;} th=Math.floor(th/2); await ns.sleep(20); }
    return started;
  };
  ...
  while(true){
    ...
    if(!state.paused){
      ...
      if(mode==='auto'){
        if(s.sec>s.min+FLAGS.secPad) mode='weaken';
        else if(s.max>0 && s.money<s.max*FLAGS.moneyThr) mode='grow';
        else mode='hack';
      }
      if(mode==='weaken') await runFill(W,[tgt]);
      else if(mode==='grow') await runFill(G,[tgt]);
      else await runFill(H,[tgt],128);
    }
