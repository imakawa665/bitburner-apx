...
export async function main(ns){
  ...
  function rootedHosts(){ const seen=new Set(); const q=['home']; const out=[]; while(q.length){ const s=q.pop(); if(seen.has(s)) continue; seen.add(s); for(const n of ns.scan(s)) q.push(n); } for(const h of seen){ try{ if(ns.getServer(h).hasAdminRights) out.push(h);}catch{} } return out; }
  ...
  function runLoop(host,target){
    ...
    const cost=ns.getScriptRam(LOOP,'home')||1.6; const th=Math.max(1, Math.floor(freeRam(host)/cost));
    if(th>0) ns.exec(LOOP,host,1,target);
  }
  while(true){
