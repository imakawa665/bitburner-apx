...
/** tools/apx-autopilot.full.v1.js  (v1.8.4 "no-crime")
 * ポリシー: SF4 未所持時の不安定さ回避のため、犯罪/Karma 関連の自動化は一切行いません。
 * - 起動時に crime/karma 系スクリプトを見つけたら停止します。
 * - それ以外の自動化（DW購入→rooter再走 / Casino条件起動 / Backdoor DOM / Study DOM / Hacknet / Spread / Micro / HUD）は従来通り。
 */
export async function main(ns){
  ...
  // --- 強制クリーンアップ（crime/karma 系は起動させない） ---
  for (const f of ['tools/apx-crime.repeat.v1.js','tools/apx-crime.repeat.dom.v1.js','tools/apx-karma.watch.v1.js']) {
    for (const p of ns.ps('home').filter(p=>p.filename===f)) ns.kill(p.pid);
  }

  const F=ns.flags([
    ['interval',5000],['goal',1e9],
  ...
  while(true){
    ...
    // Darkweb buyer: 不足クラッカーがあれば自動起動（購入で rooter 再走）
