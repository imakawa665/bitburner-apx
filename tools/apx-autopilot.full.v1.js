/** tools/apx-autopilot.full.v1.js  (v1.8.1) */
export async function main(ns){
  ...
  while(true){
    ...
    const missing=crackers.filter(c=>!exists(c)); if (missing.length>0 && exists('tools/apx-darkweb.autobuyer.v1.js')) { runOnce('tools/apx-darkweb.autobuyer.v1.js',1,'--mode','ports','--safety',1.0,'--method','auto','--autoRooterRestart'); print('darkweb buyer running; missing=', missing.join(',')); }
    const nowC=crackers.filter(exists).length; if (nowC>lastCrackers){ await restart('rooter/apx-rooter.auto.v1.js',['--interval',10000,'--log']); runOnce('tools/apx-spread.remote.v1.js',1); lastCrackers=nowC; ns.toast(`New crackers: ${nowC}/5. Rooting expanded.`,'info',4000); }
    const banned = ns.fileExists(F.banFile,'home'); if (!banned && ns.getServerMoneyAvailable('home') < Number(F.goal||1e9)) runOnce('tools/apx-casino.runner.v1.js',1,'--goal',Number(F.goal||1e9),'--minTravel',200000); else { for (const p of ns.ps('home').filter(p=>p.filename==='tools/apx-casino.runner.v1.js')) ns.kill(p.pid); }
    ...
    if (F.crimeAuto) { runOnce('tools/apx-karma.watch.v1.js',1,'--goal',Number(F.karmaGoal||-54000)); runOnce('tools/apx-crime.repeat.v1.js',1,'--autoPick',true); }
