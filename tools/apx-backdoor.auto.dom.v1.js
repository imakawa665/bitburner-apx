 ...
 * DOM を使って、到達可能な「特殊サーバ」を自動で connect→backdoor。
 * - UIロック(/Temp/apx.ui.lock.txt)で他DOMスクリプト（casino等）と排他制御。
 * - 条件: root済み + 要求Hack満たす + 未backdoor のものだけ。
 */
export async function main(ns){
  ...
  while(true){
    for(const h of specials){
      ...
      if(ns.getPlayer().skills.hacking < req) { log('skip (need hack',req,')',h); continue; }
      const path=find(h); const cmd=gen(path);
      if(!cmd) continue;
      if(!acquire()){ log('UI locked; wait'); continue; }
      ns.tprint(`[auto-backdoor] ${h} に backdoor 実行: ${cmd}`);
      await term(cmd);
      // backdoor は時間がかかるので余裕をみて待つ
      await ns.sleep(8000);
      release();
    }
