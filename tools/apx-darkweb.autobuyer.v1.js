/** tools/apx-darkweb.autobuyer.v1.js (v1.1) */
export async function main(ns) {
  ...
  let useSing=false; if(String(F.method).toLowerCase()==='singularity') useSing=true; else if(String(F.method).toLowerCase()==='dom') useSing=false; else { try{ useSing=!!(ns.singularity&&ns.singularity.purchaseProgram); }catch{ useSing=false; } }
  ...
  const tryPurchaseSing=async(prog)=>{ try{ if(ns.singularity.purchaseTor){ try{ ns.singularity.purchaseTor(); }catch{} } return !!ns.singularity.purchaseProgram(prog.name);}catch{return false;} };
  ...
  while(true){
    ...
    for(const prog of list){
      ...
      if(ok||have(prog.name)){ bought++; ns.toast(`Purchased ${prog.name}`,'success',4000); ns.tprint(`[darkweb] Purchased ${prog.name}  $${prog.cost.toLocaleString()}`); if(F.autoRooterRestart) restart(String(F.rooter||'rooter/apx-rooter.auto.v1.js'),['--interval',10000,'--log']); await ns.sleep(200); }
    }
