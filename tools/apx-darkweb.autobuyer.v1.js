 ...
 * Dark Web Auto Buyer (Singularity不要 / DOMアシスト) - v1.0
 * 詳細は配布メモ参照
 */
export async function main(ns) {
  ...
  while(true){
    ...
    for(const prog of list){
      ...
      if (ok || have(prog.name)) { justBought++; ns.toast(`Purchased ${prog.name}`, 'success', 4000); ns.tprint(`[darkweb] Purchased ${prog.name}  $${prog.cost.toLocaleString()}`); await ns.sleep(200); }
    }
