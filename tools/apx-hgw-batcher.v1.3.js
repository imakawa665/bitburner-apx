...
/** tools/apx-hgw-batcher.v1.3.js
 * Real scheduler: H -> W1 -> G -> W2 finishing in order, using scheduled workers with delay argument.
 * Args: --target <host> --hackPct 0.05 --gap 200 --lanes 2 --secMargin 0.5 --reserveHomeGB 8
 */
export async function main(ns){
  ...
  if(!ns.hasRootAccess(F.target)){ ns.tprint(`[batcher] no root on ${F.target}`); return; }
  ...
  async function prep(){
    ...
    if(s.hackDifficulty > s.minDifficulty + Number(F.secMargin||0.5)){
      ...
      ns.exec(W,'home',tw,F.target,0);
      await ns.sleep(ns.getWeakenTime(F.target)+100);
      return false;
    }
    if(s.moneyAvailable < s.moneyMax*0.99){
      const mult = Math.max(1.01, s.moneyMax / Math.max(1, s.moneyAvailable));
      const tg = Math.ceil(ns.growthAnalyze(F.target, mult));
      const tw = Math.ceil((tg*secPerGrow)/secPerWeak);
      ns.exec(G,'home',tg,F.target,0);
      ns.exec(W,'home',tw,F.target,0);
      await ns.sleep(ns.getGrowTime(F.target)+200);
      return false;
    }
    return true;
  }
  ...
  function threads(){
    const s=ns.getServer(F.target);
    const money = Math.max(1, s.moneyAvailable);
    ...
    const after = Math.max(1, money - steal);
  ...
  while(true){
    ...
    const base = Date.now() + 200; // baseline
    for(let i=0;i<lanes;i++){
      const laneOffset = i * gap;
      const fin = base + 3*gap + laneOffset; // H finishes first
      ...
      ns.exec(H,'home',hth,F.target,dH);
      ns.exec(W,'home',w1,F.target,dW1);
      ns.exec(G,'home',gth,F.target,dG);
      ns.exec(W,'home',w2,F.target,dW2);
      await ns.sleep(gap);
    }
