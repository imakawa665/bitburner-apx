...
/** Real scheduling batcher: H -> W1 -> G -> W2 */
export async function main(ns){
  ...
  async function prep(){
    ...
    if(s.hackDifficulty > s.minDifficulty + Number(F.secMargin||0.5)){
      const tw=Math.max(1, Math.ceil((s.hackDifficulty - s.minDifficulty)/secPerWeak));
      ns.exec(W,'home',tw,F.target,0);
      await ns.sleep(ns.getWeakenTime(F.target)+100); return false;
    }
    if(s.moneyAvailable < s.moneyMax*0.99){
      ...
      ns.exec(G,'home',tg,F.target,0); ns.exec(W,'home',tw,F.target,0);
      await ns.sleep(ns.getGrowTime(F.target)+200); return false;
    }
    return true;
  }
  function threads(){
