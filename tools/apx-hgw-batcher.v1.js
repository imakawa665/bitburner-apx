...
export async function main(ns) {
  ns.disableLog('sleep'); const F=ns.flags([['target','n00dles'],['hackPct',0.05],['gap',200],['loop',true]]); const t=String(F.target);
  ...
  while(true){
    ...
    const can=(file,th,...args)=>{ if(th<=0) return 0; let started=0,tryTh=th; while(tryTh>0){ const pid=ns.run(file,tryTh,...args); if(pid!==0){started=tryTh;break;} tryTh=Math.floor(tryTh/2); if(tryTh===1){ if(ns.run(file,1,...args)!==0){started=1;break;} else break;} ns.sleep(5);} return started; };
    const sleepUntil=async(ms)=>{ const d=ms-Date.now(); if(d>0) await ns.sleep(d); };
    await sleepUntil(times.h);  can('apx-h1.js', hackThreads, t);
    await sleepUntil(times.w1); can('apx-w1.js', w1, t);
    await sleepUntil(times.g);  can('apx-g1.js', growThreads, t);
    await sleepUntil(times.w2); can('apx-w1.js', w2, t);
