...
function apxFmtRam(ns, gb){
  ...
  while(true){
    try{
      ...
      const maxH=ns.getServerMaxRam('home'), usedH=ns.getServerUsedRam('home'); show("HomeRAM", `${apxFmtRam(ns,maxH)} ${(100*usedH/maxH).toFixed(1)}%`, "Home RAM");
      (function(){ const seen=new Set(); const q=['home']; let tU=0,tM=0; while(q.length){ const s=q.pop(); if(seen.has(s)) continue; seen.add(s); for(const n of ns.scan(s)) q.push(n);} for(const h of seen){ try{ const sv=ns.getServer(h); if(!sv.hasAdminRights) continue; tU+=sv.ramUsed||0; tM+=sv.maxRam||0; }catch{} } show("AllRAM", `${apxFmtRam(ns,tM)} ${(tM?(100*tU/tM).toFixed(1):'0.0')}%`, "Total rooted RAM"); })();
      const inc=ns.getTotalScriptIncome?.()||[0,0]; show("Income", apxFmtNumber(ns,inc[0],3)+'/s', "Instant income");
      const exp=ns.getTotalScriptExpGain?.()||0; show("Exp", apxFmtNumber(ns,exp,3)+'/s', "Instant EXP");
      const reserve=Number(ns.read('reserve.txt')||0); show("Reserve", reserve>0?apxFmtMoney(ns,reserve):null, "Reserved cash");
    }catch{}
    await ns.sleep(1000);
  }
}
