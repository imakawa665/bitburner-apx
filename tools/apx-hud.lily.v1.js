/** apx-hud.lily.v1.js  (v1.1.2 HOTFIX) */
export async function main(ns) {
  ...
  const exists=(h)=>{ try{ return ns.serverExists(h); }catch{ return false; } };
  const safeReq=(h)=> exists(h) ? ns.getServerRequiredHackingLevel(h) : 1e9;
  const safeMax=(h)=> exists(h) ? (ns.getServerMaxMoney(h)||0) : 0;
  ...
  const best=()=>{ const me=ns.getPlayer().skills.hacking; let b='',score=-1; for(const h of servers()){ if(h==='home'||h==='darkweb') continue; if(!exists(h)) continue; if(!ns.hasRootAccess(h)) continue; if(safeReq(h)>me) continue; const m=safeMax(h); if(m<=0) continue; const req=safeReq(h)||1; const s=m/(1+req); if(s>score){score=s;b=h;} } return b||'n00dles'; };
  const backlist=['CSEC','avmnite-02h','I.I.I.I','run4theh111z','w0r1d_d43m0n'].filter(h=>exists(h));
  const nextBack=()=>{ const me=ns.getPlayer().skills.hacking; for(const h of backlist){ if(!exists(h)) continue; const s=ns.getServer(h); if(!s||s.backdoorInstalled) continue; if(safeReq(h)>me) continue; if(!ns.hasRootAccess(h)) continue; const path=pathTo(h); const tip=`Path: ${path?path.join(' -> '):'N/A'}\nReqHack: ${safeReq(h)}`; return {host:h,path,tip}; } return null; };

  let cachedCmd = ""; const nextCmdCell = nodes['NextCmd'][5]; nextCmdCell.style.cursor='pointer';
  ...
  while(true){
    ...
    const sv=servers(); const alive=sv.filter(exists); const rooted=alive.filter(h=>ns.hasRootAccess(h)).length; set('Network', `${alive.length}/${rooted}`, '全サーバ数(存在) / root済み数');
    const p=(ns.getPurchasedServers?.()??[]).filter(exists); const lim=ns.getPurchasedServerLimit(); const tRam=p.reduce((a,h)=>a+(exists(h)?ns.getServerMaxRam(h):0),0); set('Purchased', `${p.length}/${lim} (${tRam}GB)`, '買収サーバ数 / 上限 (総RAM)');
    const inc=ns.getTotalScriptIncome()[0]; set('Income', `$${(inc).toFixed(2)}/s`, '全スクリプトの瞬間収益');
    const nb=nextBack(); if(nb){ set('Backdoor', nb.host, nb.tip); const cmd=gen(nb.path)||""; cachedCmd=cmd; set('NextCmd', cachedCmd?'Click to Copy':'-', cachedCmd); } else { cachedCmd=""; set('Backdoor','-','候補なし'); set('NextCmd','-',''); }
    await ns.sleep(1000);
  }
}
