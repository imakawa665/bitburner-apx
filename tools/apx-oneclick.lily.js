...
export async function main(ns) {
  ...
  const runOnce=(f,th=1,...args)=>{ if(!exists(f)){ ns.tprint(`[oneclick] missing: ${f}`); return 0;} if(isAny(f)) return 1; const pid=ns.run(f,th,...args); if(pid===0) ns.tprint(`[oneclick] failed: ${f}`); else print('start',f,'pid',pid,'args',JSON.stringify(args)); return pid?1:0; };
  ...
  if (pf){
    if(pf==='lowram'){
      ...
      F.daemon=true; F.hash=false; F.withBatcher=false;
      ns.tprint(`[profile] casino: カジノ最優先（daemonでrunner起動）`);
    } else if(pf==='karma'){
  ...
  ns.tprint(`[oneclick] 起動完了: profile=${pf||'-'} crimeLoop=${F.crimeLoop} stanek=${F.stanekCharge} factionAssist=${F.factionAssist} HUD=${F.hud} rooter=${F.rooter} micro=${F.micro} pserv=${F.pserv} hacknet=${F.hacknet} share=${F.share} spread=${F.spread} batcher=${F.withBatcher} lanes=${F.lanes} backdoor=${F.backdoor} hash=${F.hash} daemon=${F.daemon}`);
}
