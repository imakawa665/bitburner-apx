...
export async function main(ns) {
  ...
  const F = ns.flags([
    ['budget', 0.5], ['minRam', 64], ['maxRam', 8192],
    ['target',''], ['prefix','px'], ['keep', false], ['interval', 15000]
  ]);
  ...
  const ensureLoop = async (host, target) => {
    if (!ns.isRunning(LOOP, host)) {
      ...
      if (files.length) await ns.scp(files, host);

      const max = ns.getServerMaxRam(host);
      let cost = ns.getScriptRam(LOOP, 'home');
      if (!isFinite(cost) || cost <= 0) cost = 1.6; // フェイルセーフ
      let t = Math.max(1, Math.floor(max / cost));
      if (!isFinite(t) || t < 1) t = 1;

      ns.exec(LOOP, host, t, '--target', target);
    }
  };
