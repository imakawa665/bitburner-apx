 ...
 * 買収サーバの自動購入・配置（単発）
 * LOG: 重要イベントで ns.print を追加（ロジック変更なし）
 * @param {NS} ns
 */
export async function main(ns) {
  ...
  const need=Math.max(0,limit-have.length); let deleted=0; if(need===0 && !F.keep){ const infos=have.map(h=>({h,r:ns.getServerMaxRam(h)})).sort((a,b)=>a.r-b.r); for(const info of infos){ if(info.r>=bestRam) continue; if(ns.ps(info.h).length>0){ ns.killall(info.h); await ns.sleep(10);} if(ns.deleteServer(info.h)){ deleted++; log('deleted',info.h,'for upgrade',bestRam); break; } } }
  ...
  const max=ns.getServerMaxRam(host), sCost=ns.getScriptRam(LOOP,'home'); let loopCost=sCost; if(!isFinite(loopCost)||loopCost<=0) loopCost=1.6; let th=Math.max(1,Math.floor(max/loopCost)); if(!isFinite(th)||th<1) th=1;
