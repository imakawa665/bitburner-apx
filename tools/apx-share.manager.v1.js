 ...
 * 目的: REP強化用に network 全体で Share を最大化。
 * 仕様: すべてのルート済みサーバに `tools/apx-share.nano.v1.js` を1本だけ起動し、余剰RAMをすべて割当。
 * 引数: --reserveHomeGB  (Home の予約RAM, 既定 8)  --includeHacknet (false)
 */
export async function main(ns){
  ...
  if(!ns.fileExists(SHARE,'home')){ ns.tprint('[share.mgr] missing',SHARE); return; }
  ...
  function freeRam(host){
    ...
    let free=max-used; if(host==='home') free=Math.max(0,free-Math.max(0,Number(F.reserveHomeGB)||8));
  ...
  while(true){
    for(const h of rooted()){
      const free=freeRam(h);
      const cost=ns.getScriptRam(SHARE,'home')||1.6;
      const th=Math.floor(free/cost);
      if(th<=0) continue;
      if(runningOn(h)){ continue; }
      try{
        if(h!=='home' && !ns.fileExists(SHARE,h)) await ns.scp(SHARE,h);
        const ok=ns.exec(SHARE,h,th);
        if(ok) log('start',h,'th',th);
      }catch{}
      await ns.sleep(10);
    }
