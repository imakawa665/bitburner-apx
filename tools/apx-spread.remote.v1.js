...
export async function main(ns) {
  ...
  for(const h of hosts){
    if(files.length) { await ns.scp(files,h); log('scp',h); }
    const max=ns.getServerMaxRam(h), used=ns.getServerUsedRam(h); const free=Math.max(0,max-used); const cost=ns.getScriptRam('workers/apx-loop-hgw.nano.js','home')||1.6; const th=Math.max(1,Math.floor(free/cost));
